<script lang="ts">
  ...
  async function updateQuestion(event: KeyboardEvent) {
    console.log($state.question);
    socket.updateQuestion($state.question);
  }

  function signalTyping() {
    if (myAnswer.trim().length) socket.signalTyping();
  }
...
<main>
  <h1
    ...
    bind:textContent={$state.question}
    on:keyup={updateQuestion}
  />
  {#if numberTyping == 1}
      ...
      {#if myAnswer.length >= 3}
        <label for="paperSwitch1" class="paper-switch-tile">
          ...
          <div class="paper-switch-tile-card border">
            <div class="paper-switch-tile-card-front border">Send</div>
            <div class="paper-switch-tile-card-back border background-success">
              Sent!
            </div>
          </div>
        </label>
